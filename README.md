...
A modernized MediaWiki extension rewrite (wip) for sending notifications to a Discord webhook from MediaWiki. When certain events occur on your wiki, such as new edits, they can be sent as messages to a Discord channel via a webhook.
...
```bash
git clone --depth=100 --branch "REL1_44" https://github.com/jhnhnck/mediawiki-extensions-Discord ./extensions/NovaDiscord;
```

2. Configure the extension within your `LocalSettings.php` file:
...
| --- | --- | --- | --- |
| `$wgDiscordWebhookURL` **(required)** | string\|array | [Webhook URL](https://support.discord.com/hc/en-us/articles/228383668-Intro-to-Webhooks) for Discord channel | *none* |
