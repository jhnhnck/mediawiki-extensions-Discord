...
class PageSaveCompleteHookTest extends NovaDiscordIntegrationTestCase {
    ...
    public function testPageEditSendsWebhook(): void {
        ...
        $username = $user->getUser()->getName();
        $userScored = str_replace(' ', '_', $username);
        $expectedPayload = '{"content":"[' . $username . '](<https:\/\/novadiscord.local\/wiki\/User:' . $userScored . '>) ([t](<https:\/\/novadiscord.local\/wiki\/User_talk:' . $userScored . '>)|[c](<https:\/\/novadiscord.local\/wiki\/Special:Contributions\/' . $userScored . '>)) edited [NovaTestPageEdit](<https:\/\/novadiscord.local\/wiki\/NovaTestPageEdit>) ([diff](<https:\/\/novadiscord.local\/index.php?title=NovaTestPageEdit&diff=prev&oldid=2>)) (+44) `Added content`","allowed_mentions":{"parse":[]}}';
